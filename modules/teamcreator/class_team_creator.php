...
class TeamCreator implements ModuleInterface
{
	...
	private static function addPlayer($pos, $d, $isStarPlayer) {
		...
		$p['keyword'] = '('.implode(', ', keywordsTrans($d['keyword'])).')';
		$p['ma'] = $d['ma'];
		$p['st'] = $d['st'];
		$p['ag'] = $d['ag']."+";
	...
	public static function handlePost($cid) {
		...
		foreach ($race['players'] as $pos => $d) {
			...
			for($i = 0; $i < $qty; $i++) {
				$treasury -= $d['cost'];
				$bg -= $d['is_bigguy'];
				if (in_array('134',$d['def'])) {  //check if player has insignificant trait
					$insigplayers += 1;
				}
				...
				$players[] = $player;
				//$insigplayers = count($is_insig);
				$notinsigplayers = (count($players) - $insigplayers);
			}
			$idx++;
		}
		...
		if ($ca < 0) {
			$errors[] = $lng->getTrn('tooManyCatchers', 'TeamCreator');
		}
		if ($th < 0) {
...
EOQ;
	...
	public static function newTeam($cid) {
		...
		$txtTooFewInsignificant = $lng->getTrn('tooFewInsignificant', 'TeamCreator');
		...
		function changeRace(raceId) {
			...
			for (i = 0; i < race["player_count"]; i++) {
				...
				addCellToRow(row, player["keyword"], 1);
...
EOQ;
		...
		$txtKey = $lng->getTrn('common/keywords');
		...
		echo <<<EOQ
		<td align="right" id="indTxt"></td>
		<td><input type="hidden" id="induce" onChange="changeInduce(this.checked)" /><input type="hidden" id="oldInduce" value="false" /></td>
