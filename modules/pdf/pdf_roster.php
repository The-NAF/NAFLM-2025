...
// Get Team League and Special Rules
// Build league display text
$teamleague_available = leaguesTrans($race->team_league);
$teamleague_text = $teamleague_available;

if (strlen($team->getLeagueoptions()) > 1 && strlen($team->getLeaguechosen()) >= 1) {
    $fr_text = 'profile/team/box_tm/favrule/r'.$team->getLeaguechosen();
    $chosen_league = $lng->getTrn($fr_text);
    $teamleague_text = $chosen_league;
}

// Build special rules display text
$teamspecialrules_text = '';
if (strlen($team->getTeamrules()) > 0) {
    $teamspecialrules_text = specialsTrans($race->special_rules);
} else {
    if ($team->f_rname == 'Norse' && $team->getFavrulechosen() == 15) {
        $teamspecialrules_text = 'Favoured of Khorne';
    } else {
        $teamspecialrules_text = 'none';
    }
}

// Add chosen favorite rule if exists
if (strlen($team->getFavruleoptions()) != 0 && strlen($team->getFavrulechosen()) >= 1) {
    $fr_text = 'profile/team/box_tm/favrule/r'.$team->getFavrulechosen();
    $chosen_favrule = $lng->getTrn($fr_text);
    if ($teamspecialrules_text != 'none') {
        $teamspecialrules_text .= ', ' . $chosen_favrule;
    } else {
        $teamspecialrules_text = $chosen_favrule;
    }
}
...
$pdf->SetFont('Tahoma','B',14);
$pdf->Cell(240, 20, utf8_decode($team->name), 0, 0, 'L', false, '');

$pdf->SetFont('Tahoma','',12);
$pdf->Cell(30, 20, "Race:", 0, 0, 'R', false, '');
$pdf->Cell(110, 20, ($team->f_rname), 0, 0, 'L', false, '');
$pdf->Cell(30, 20, "Tier:", 0, 0, 'R', false, '');
$pdf->Cell(10, 20, $teamtier, 0, 0, 'L', false, '');

// Team League and Special Rules in two rows (same total height as header = 20)
$currentx_league = $currentx + 8 + 240 + 30 + 110 + 30 + 10 + 20;
$pdf->SetXY($currentx_league, $currenty);
$pdf->SetFont('Tahoma','',8);

// Truncate if too long (adjust 80 to your preference for max characters)
$display_league = (strlen($teamleague_text) > 80) ? substr(utf8_decode($teamleague_text), 0, 77) . '...' : utf8_decode($teamleague_text);
$display_rules = (strlen($teamspecialrules_text) > 80) ? substr(utf8_decode($teamspecialrules_text), 0, 77) . '...' : utf8_decode($teamspecialrules_text);

$pdf->Cell(45, 10, "Team League:", 0, 0, 'R', false, '');
$pdf->Cell(95, 10, $display_league, 0, 0, 'L', false, '');

// Second row for Special Rules
$pdf->SetXY($currentx_league, $currenty + 10);
$pdf->SetFont('Tahoma','',8);
$pdf->Cell(45, 10, "Special Rules:", 0, 0, 'R', false, '');
$pdf->Cell(95, 10, $display_rules, 0, 0, 'L', false, '');

// Head Coach on the far right
$pdf->SetXY($currentx_league + 160, $currenty);
$pdf->SetFont('Tahoma','',12);
$pdf->Cell(170, 20, ("Head Coach: " . utf8_decode($team->f_cname)), 0, 0, 'R', false, '');
...
$pdf->Cell(80, $h, 'Name', 1, 0, 'L', true, '');
$pdf->Cell(80, $h, 'Position', 1, 0, 'L', true, '');
$pdf->Cell(84, $h, 'Keywords', 1, 0, 'L', true, '');
...
$pdf->Cell(245, $h, 'Skills and Injuries', 1, 0, 'L', true, '');
...
$pdf->Cell(36, $h, 'Value', 1, 0, 'C', true, '');
...
foreach ($players as $p) {
  ...
  $keywordstr = $p->getKeywordsStr(false);
  
  // Concatenate team captain, skills, hatred, upgrades and injuries
  ...
  //begin string build based on four optional strings
	if ($capstr == '') {  // is not captain
		if ($skillstr == '') {  // no skills
			if ($hatestr == '') {  // no hatred
				if ($injstr == '') {  // no injuries
					$skills_injuries='';  // nothing
				} else {
					$skills_injuries=$injstr;  // only injuries
				}
			} else {
				if ($injstr == '') {  // no injuries
					$skills_injuries='Hatred (' . $hatestr . ')';  // just hatred
				} else {
					$skills_injuries='Hatred (' . $hatestr . '), ' . $injstr;  // hatred + injuries
				}
			}
		} else {
			if ($hatestr == '') {  // no hatred
				if ($injstr == '') {  // no injuries
					$skills_injuries=$skillstr;  // just skills
				} else {
					$skills_injuries=$skillstr . ', ' . $injstr;  // skills + injuries
				}
			} else {
				if ($injstr == '') {  // no injuries
					$skills_injuries=$skillstr . ', Hatred (' . $hatestr . ')';  // skill + hatred
				} else {
					$skills_injuries=$skillstr . ', Hatred (' . $hatestr . '), ' . $injstr;  // skills + hatred + injuries
				}
			}
		}	
	} else {  //is captain
		if ($skillstr == '') {  // no skills
			if ($hatestr == '') {  // no hatred
				if ($injstr == '') {  // no injuries
					$skills_injuries=$capstr;  // just captain
				} else {
					$skills_injuries=$capstr . ', ' . $injstr;  // captain + injuries
				}
			} else {
				if ($injstr == '') {  // no injuries
					$skills_injuries=$capstr . ', Hatred (' . $hatestr . ')';  // captain + hatred
				} else {
					$skills_injuries=$capstr . ', Hatred (' . $hatestr . '), ' . $injstr;  // captain + hatred + injuries
				}
			}
		} else {
			if ($hatestr == '') {  // no hatred
				if ($injstr == '') {  // no injuries
					$skills_injuries=$capstr . ', ' . $skillstr;  // captain + skills
				} else {
					$skills_injuries=$capstr . ', ' . $skillstr . ', ' . $injstr;  // captain + skills + injuries
				}
			} else {
				if ($injstr == '') {  // no injuries
					$skills_injuries=$capstr . ', ' . $skillstr . ', Hatred (' . $hatestr . ')';  // captain + skill + hatred
				} else {
					$skills_injuries=$capstr . ', ' . $skillstr . ', Hatred (' . $hatestr . '), ' . $injstr;  // captain + skills + hatred + injuries
				}
			}
		}	
	}  
  ...
  $pp = array('nr'=>$p->nr, 'name'=>utf8_decode($p->name), 'pos'=>$p->position, 'keyw'=>utf8_decode($keywordstr), 'ma'=>$p->ma, 'st'=>$p->st, 'ag'=>$p->ag, 'pa'=>$p->pa, 'av'=>$p->av, 'skills'=>utf8_decode($skills_injuries), 'inj'=>$inj,
     'cp'=>$p->mv_cp, 'td'=>$p->mv_td, 'int'=>$p->mv_intcpt, 'cas'=>$p->mv_cas, 'mvp'=>$p->mv_mvp, 'misc'=>$p->mv_misc, 'spp'=>$p->mv_spp, 'value'=>$pdf->Mf($p->value));
  $sum_spp+=$p->mv_spp;
  $sum_pvalue+=$p->value;
  $sum_players++;
  $sum_cp+=$p->mv_cp;
  ...
  $sum_misc+=$p->mv_misc;
    ...
    if (T_PDF_ROSTER_SET_EMPTY_ON_ZERO) {
        foreach (array('cp','td','int','cas','mvp','misc','spp') as $f) {
            if ($pp[$f] == 0) {
                $pp[$f] = '';
            }
        }
...
$pp = array('nr'=>'', 'name'=>'', 'pos'=>'', 'keyw'=>'', 'ma'=>'', 'st'=>'', 'ag'=>'', 'pa'=>'', 'av'=>'', 'skills'=>'', 'inj'=>'',
            'cp'=>'', 'td'=>'', 'int'=>'', 'cas'=>'', 'mvp'=>'', 'misc'=>'', 'spp'=>'', 'value'=>'');
...
if ($_POST) {
  ...
  while (list($key, $val) = each($postvars)) {
    ...
    elseif ($key == 'Prayers to Nuffle') { $ind_ptn = (int) $val; continue; }
    ...
    elseif ($key == 'Blitzer\'s Best Kegs') { $ind_babes = (int) $val; continue; }
    ...
    elseif ($key == 'Sports-Wizard') { $ind_wiz = (int) $val; continue; }
    ...
    elseif ($key == 'Bretonnian Pastries') { $ind_bpast = (int) $val; continue; }
    elseif ($key == 'Bretonnian Damsel') { $ind_bdam = (int) $val; continue; }
    elseif ($key == 'Canopic Jar') { $ind_cjar = (int) $val; continue; }
    elseif ($key == 'Biased Referee: Dodgy League Rep') { $ind_bref = (int) $val; continue; }
...
$r=$team->f_rname;
if (($r == 'Nurgle') || ($r == 'Tomb Kings') || ($r == 'Necromantic Horror') || ($r == 'Shambling Undead')) {
...
// Checking game data if cheaper bribes for Dwarf, Goblin, Snotling, Black Orc and Underworld (bribery & corruption teams)
$bribe_cost = $inducements['Bribes'][(($r == 'Dwarf'||$r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? 'reduced_cost' : 'cost')];
// Checking game data if more bribes available for Dwarf, Goblin, Snotling, Black Orc and Underworld (bribery & corruption teams)
$bribe_max = (($r == 'Dwarf'||$r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? '6' : '3');
// Checking game data if cheaper biased referee for Dwarf, Goblin, Snotling, Black Orc and Underworld (bribery & corruption teams)
$biasedref_cost = $inducements['Biased Referee: Dodgy League Rep'][(($r == 'Dwarf'||$r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? 'reduced_cost' : 'cost')];
// Checking game data if cheaper Biased Referee: Jorm the Ogre for Dwarf, Goblin, Snotling, Black Orc and Underworld (bribery & corruption teams)
$bjorm_cost = $inducements['Biased Referee: Jorm the Ogre'][(($r == 'Dwarf'||$r == 'Goblin'||$r == 'Snotling'||$r == 'Black Orc'||$r == 'Underworld Denizens') ? 'reduced_cost' : 'cost')];
...
if (isset($ind_ptn)) { $ind_cost += $ind_ptn*$inducements['Prayers to Nuffle']['cost']; $ind_count += 1; }
...
if (isset($ind_babes)) { $ind_cost += $ind_babes*$inducements['Blitzer\'s Best Kegs']['cost']; $ind_count += 1; }
...
if (isset($ind_wiz)) { $ind_cost += $ind_wiz*$inducements['Sports-Wizard']['reduced_cost']; $ind_count += 1; }
...
if (isset($ind_bpast)) { $ind_cost += $ind_bpast*$inducements['Bretonnian Pastries']['reduced_cost']; $ind_count += 1; }
if (isset($ind_bdam)) { $ind_cost += $ind_bdam*$inducements['Bretonnian Damsel']['reduced_cost']; $ind_count += 1; }
if (isset($ind_cjar)) { $ind_cost += $ind_cjar*$inducements['Canopic Jar']['reduced_cost']; $ind_count += 1; }
...
$ind_display_counter = 0;
  ...
  if (isset($ind_babes)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Blitzer\'s Best Kegs (0-2):', $ind_babes, $pdf->Mf($inducements['Blitzer\'s Best Kegs']['cost']));}
  if (isset($ind_babes)) { $ind_display_counter += 1;}
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;} 
  if (isset($ind_bribes)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Bribes (0-'.$bribe_max.'):', $ind_bribes, $pdf->Mf($bribe_cost));}
  ...
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;} 
  if (isset($ind_ptn)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Prayers to Nuffle (0-3):', $ind_ptn, $pdf->Mf($inducements['Prayers to Nuffle']['cost']));}
  if (isset($ind_ptn)) { $ind_display_counter += 1;}
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;} 
  if (isset($ind_sac)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Part-time Assistant Coaches (0-5):', $ind_sac, $pdf->Mf($inducements['Part-time Assistant Coaches']['cost']));}
  if (isset($ind_sac)) { $ind_display_counter += 1;}
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;}
  if (isset($ind_tem)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Temp Agency Cheerleaders (0-5):', $ind_tem, $pdf->Mf($inducements['Temp Agency Cheerleaders']['cost']));}
  ...
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;}
  if (isset($ind_wiz)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Sports-Wizard (0-1):', $ind_wiz, $pdf->Mf($inducements['Sports-Wizard']['cost']));}
  ...
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;}
  if (isset($ind_bpast)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Bretonnian Pastries (0-1):', $ind_bpast, $pdf->Mf($inducements['Bretonnian Pastries']['reduced_cost']));}
  if (isset($ind_bpast)) { $ind_display_counter += 1;}
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;}
  if (isset($ind_bdam)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Bretonnian Damsel (0-1):', $ind_bdam, $pdf->Mf($inducements['Bretonnian Damsel']['reduced_cost']));}
  if (isset($ind_bdam)) { $ind_display_counter += 1;}
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;}
  if (isset($ind_cjar)) { $pdf->print_inducements($currentx, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Canopic Jar (0-1):', $ind_cjar, $pdf->Mf($inducements['Canopic Jar']['reduced_cost']));}
  if (isset($ind_cjar)) { $ind_display_counter += 1;}
  if ($ind_display_counter == 9 ) {$currentx += 250; $currenty = 435;}
