...
class IndcPage implements ModuleInterface
{
    ...
    static public function main($argv)
    {
        ...
        global $lng, $stars, $DEA, $rules, $skillarray, $inducements, $racesNoApothecary, $starpairs, $specialruleididx;
        ...
        foreach ($stars as $s => $d) {
            ...
            if ($rules['megastars'] == 0 || ($d['megastar'] == 0 && $rules['megastars'] == 1)) {
				...
				if (strlen($t->getLeaguechosen()) == 0 ) { //defining team league rules 
					$teamdefrules = explode(",", $t->getLeagueoptions()); 
				} else {
					$teamdefrules = explode(",", $t->getLeaguechosen());
				}
				$teamdfavrule = explode(",", $t->getFavrulechosen()); //defining team favoured rule
				$teamdspecrule = explode(",", $t->getTeamrules()); //defining team special rule
				$allteamrules = array_merge($teamdefrules, $teamdfavrule, $teamdspecrule); //combining league, special and favoured
...
<?php
		...
		//General Inducements
		//print '<tr><td><b>General Inducements<b></td></tr>';
