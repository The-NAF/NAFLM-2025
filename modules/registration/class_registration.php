...
class Registration implements ModuleInterface
{
    ...
    function __construct($username, $password = "", $email = "", $leagueId, $form = "register") {
        ...
        switch ( $form ){
            ...
            case "register":
                ...
                if(!$leagueId) {
                    $this->error = 'You must select a league. If your league isn\'t listed, talk with an admin!';
                    return false;                    
                }
...
#                 mysql_real_escape_string($this->username), $this->password, mysql_real_escape_string($this->email), NOT_ACTIVATED, ACCESS_LEVEL );
    ...
    private static function submitForm($username, $password, $email, $league) {

        $register = new Registration($username, $password, $email, $league);
        if ( !$register->error )
        {
            Print SUCCESS_MSG;
            unset($register);
    ...
    public static function main($argv) {
        ...
        if ( isset($_POST['new_name']) && isset($_POST['new_mail']) && isset($_POST['new_passwd']) )
        {
            ...
            self::submitForm($username, $password, $email, $league);
            return true;
        }
        if ( $form == "forgot" )
        {
    ...
    */
    ...
    function activateUser() {
		...
		if ( $c->retired == 2 )
		{
			...
			// Set home league based on their membership
			$league_query = mysql_query("SELECT lid FROM memberships WHERE cid = $coach_id LIMIT 1");
			if ($league_result = mysql_fetch_assoc($league_query)) {
				$home_lid = $league_result['lid'];
				
				// Get current settings
				$current_settings = get_alt_col('coaches', 'coach_id', $coach_id, 'settings');
				
				// Add or update home_lid in settings
				if (empty($current_settings)) {
					$new_settings = "home_lid=$home_lid";
				} else {
					// Check if home_lid already exists
					if (strpos($current_settings, 'home_lid=') !== false) {
						// Update existing home_lid
						$new_settings = preg_replace('/home_lid=\d+/', "home_lid=$home_lid", $current_settings);
					} else {
						// Append home_lid (no leading comma needed since current_settings already has content)
						$new_settings = $current_settings . ",home_lid=$home_lid";
					}
				}
				
				mysql_query("UPDATE coaches SET settings = '".mysql_real_escape_string($new_settings)."' WHERE coach_id = $coach_id");
			}
		}
		else
		{
