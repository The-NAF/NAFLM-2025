    ...
    */
    ...
    switch ($_POST['type']) {
        ...
        case 'disp_access_levels':
            ...
            break;
		case 'dc':
			// Handle delete coach for retired users (registration rejection)
			if (isset($_POST['name'])) {
				if (is_numeric($cid = get_alt_col('coaches', 'name', $_POST['name'], 'coach_id'))) {
					$c = new Coach($cid);
					if (!$coach->mayManageObj(T_OBJ_COACH, $cid)) {
						status(false, 'You do not have permissions to manage the selected coach.');
					} else {
        ...
        <div class="boxBody">
            <?php
                ...
                if(empty($retiredCoaches)) {
                    ...
                    <script>
                        function confirmDelete(coachName) {
