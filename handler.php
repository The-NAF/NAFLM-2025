...
switch ($_GET['type'])
{
	...
	case 'teamcreator':
		...
		break;
	/* Tournament Team Creator */
	case 'tournamentteam':
		Module::run('TournamentTeam', array('builder'));
		break;
	/* Tournament Team PDF */
	case 'tournamentteam_pdf':
		// Don't use Module::run - call directly
		require_once('modules/tournamentteam/class_tournament_team.php');
		
		if (class_exists('TournamentTeamPDF')) {
			echo "Class found, calling main...<br>";
			TournamentTeamPDF::main(array());
		} else {
			die("TournamentTeamPDF class not found!");
		}
		exit;
		break;
	/* Calculator */
	case 'calculator':
		Module::run('Calculator', array('calculator'));
		break;
	/* League Preferences */
	case 'leaguepref':
