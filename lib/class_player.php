...
class Player
{
    ...
    function __construct($player_id) {
        ...
        $result = mysql_query("SELECT player_id,
            game_data_players.qty AS 'qty', game_data_players.pos AS 'pos', game_data_players.skills AS 'def_skills', game_data_players.keywords AS 'keywords', 
            game_data_players.ma AS 'def_ma', game_data_players.st AS 'def_st', game_data_players.ag AS 'def_ag', game_data_players.pa AS 'def_pa', game_data_players.av AS 'def_av', game_data_players.is_bigguy AS 'is_bigguy'
        ...
        */
        ...
        $this->keywords = empty($this->keywords) ? array() : explode(',', $this->keywords);
        $this->getHatred();
        $this->setSkills();
        ...
        $this->can_firecap          = !in_array($this->status, array(DEAD, RETIRED)) && ($this->inj_ma > 0 || $this->inj_ag > 0 || $this->inj_st > 0 || $this->inj_pa > 0 || $this->inj_av > 0 || $this->inj_ni > 0);
        ...
        $this->is_captain   		= ($this->is_captain == 1);
        $this->is_bigguy   			= ($this->is_bigguy == 1);
        /*
            Misc
        */
    ...
    public function setChoosableSkills() {
		...
		if(!in_array("110",$current_skills)) {# check if player has throw team mate.
			$illegal_skills = array_merge($illegal_skills, array('58','63'));# add strong arm and bullseye to illegal skills if they do not.
        }
    ...
    public function getSpecialsStr($HTML = false) {
		...
		if ($HTML) { $specials = array_strpack('<i>%s</i>', $specialrules); }
    }
    
    public function getKeywordsStr($HTML = false) {
        /**
         * Compiles keywords string.
         **/
        $keywords = keywordsTrans($this->keywords);
		return '('.implode(', ',$keywords).')';
    }
    
    public function getHatredStr($HTML = false) {
        /**
         * Compiles hatred string.
         **/
        $hatred = keywordsTrans($this->hatred);
        //if ($HTML) { $hatred = array_strpack('<i>%s</i>', $hatred); }
		return implode(', ',$hatred);
		//return '('.implode(', ',$hatred).')';
    }
    ...
    private function _getInjHistory() {
        $injs = array();
        $stats = array();
        $query = "SELECT inj, agn, f_match_id AS 'mid',  mvp, cp, td, intcpt, bh, si, ki, misc FROM match_data, matches WHERE f_match_id = match_id AND f_player_id = $this->player_id AND (inj != ".NONE." OR agn != ".NONE.") ORDER BY date_played DESC";
        if (($result = mysql_query($query)) && mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_assoc($result)) {
                $stats[] = $row;
                $tmp = array();
                foreach (array('inj', 'agn') as $inj) {
                    if ($row[$inj] != NONE) {
