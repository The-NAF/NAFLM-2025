...
class Team
{
	...
	//team lueague update - harcoded to assign fav of khorne fav rule to norse teams that select chaos clash league
    public function selectLeague($teamleague) {
		$teamid = $this->team_id;
        $query = "UPDATE teams SET teamleague = $teamleague, fav_rule = CASE WHEN $teamleague = 1 AND f_rname = 'Norse' THEN 15 ELSE fav_rule END WHERE team_id = $teamid";
    ...
    public function selectCaptain($rule) {
		$teamid = $this->team_id;
        $query = "UPDATE teams SET fav_rule = $rule WHERE team_id = $teamid";
        return mysql_query($query);
    }
    ...
    public function resetLeague() {
		$teamid = $this->team_id;
        $query = "UPDATE teams SET teamleague = NULL WHERE team_id = $teamid";
        return mysql_query($query);
    }
	...
	public function getLeagueoptions() {
		$raceid = $this->f_race_id;
		$roptions = "";
        $query = "SELECT team_league FROM races WHERE race_id = $raceid";
        $result = mysql_query($query);
        if ($row = mysql_fetch_array($result)){
            $roptions = $row['team_league'];
        }
        return $roptions;
    }
	
	public function getLeaguechosen() {
		$teamid = $this->team_id;
        $rValue = "";
		$query = "SELECT teamleague FROM teams WHERE team_id = $teamid";
        $result = mysql_query($query);
        if ($row = mysql_fetch_array($result)){
            $rValue = $row['teamleague'];
        }
        return $rValue;
    }
	...
	public function getTeamcaptain() {
		$teamid = $this->team_id;
        $rValue = "";
		$query = "SELECT player_id FROM players WHERE owned_by_team_id = $teamid AND is_captain= 1 AND date_sold IS NULL AND date_died IS NULL AND date_retired IS NULL";
        $result = mysql_query($query);
        if ($row = mysql_fetch_array($result)){
            $rValue = $row['player_id'];
        }
        return $rValue;
    }
