...
function leaguesTrans($str) {
    // Translates a commaseperated string of special rules IDs to a comma (plus space) seperated rule name string.
    $trans = array_map(create_function('$s', 'global $specialruleididx; return $specialruleididx[$s];'), is_array($str) ? $str : explode(',', $str));
    return is_array($str) ? $trans : implode(' or ', $trans);
}

function keywordsTrans($str) {
    // Translates a commaseperated string of keyword IDs to a comma (plus space) seperated rule name string.
    $trans = array_map(create_function('$s', 'global $playerkeywordsididx; return $playerkeywordsididx[$s];'), is_array($str) ? $str : explode(',', $str));
    return is_array($str) ? $trans : implode(', ', $trans);
}
