...
class Race_HTMLOUT extends Race
{
	public static function profile($rid) {
		global $lng, $DEA, $stars, $specialruleididx, $playerkeywordsididx, $rules, $racesNoApothecary;
		$race = new Race($rid);
		...
		<ul>
			...
			if (in_array($roster['other']['race_id'], $racesNoApothecary)) {
				echo $lng->getTrn('common/no');
				} else {
				echo $lng->getTrn('common/yes');
				}
			?></li>		
			<li><b><?php echo $lng->getTrn('common/maxbigguys')?>:</b> <?php echo $roster['other']['bigguy_qty'];?></li>
			<li><b><?php echo $lng->getTrn('common/teamleague')?>:</b> <?php echo leaguesTrans($race->team_league);?></li>
			<li><b><?php echo $lng->getTrn('common/specialrules')?>:</b> 
			<?php if ($roster['other']['race_id'] == T_RACE_NORSE) {
				echo $lng->getTrn('common/norseifchaos');
				}
				else {
					echo specialsTrans($race->special_rules);
					if (strlen($race->getraceFavruleoptions()) >= 1 ) {
		...
		foreach ($roster['players'] as $player => $d) {
			...
			$p->keywords = '('.implode(', ', keywordsTrans($p->keyword)).')';
		...
		if ($roster['other']['format'] == 'BB') {
			...
			foreach ($stars as $s => $d) {  
				...
				$teamdefleagues = $roster['other']['team_league'];//defining team chosen rule
				$allteamrules = array_merge($teamdefrules, $teamdefleagues); //combining standard and chosen
