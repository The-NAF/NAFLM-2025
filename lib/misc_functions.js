...
function addHatredKeywordsToOpponent(opponentTeamId, keywordIds)
{
    /*
        Adds any of keywordIds (a star's phpStars[...]['keyword'] array, which mixes positional and non-positional
        ids) that are actual Hatred keywords (present in phpHatNames) as an <option> to every Hatred ("hat_...")
        select belonging to opponentTeamId's own player rows — the tables server-rendered with data-team="<id>" in
        class_match_htmlout.php's report(). Silently skips ids already offered there (eg. already on the roster, or
        already added by an earlier star), so re-running this never creates duplicate options.
    */
    if (!keywordIds) { return; }
    var tables = document.querySelectorAll('table[data-team="' + opponentTeamId + '"]');
    for (var ki in keywordIds) {
        var kwId = keywordIds[ki];
        if (!(kwId in phpHatNames)) { continue; } // A positional keyword (or unknown id): not a Hatred option.
        for (var ti = 0; ti < tables.length; ti++) {
            var hatSelects = tables[ti].querySelectorAll("select[name^='hat_']");
            for (var hi = 0; hi < hatSelects.length; hi++) {
                var sel = hatSelects[hi];
                var already = false;
                for (var oi = 0; oi < sel.options.length; oi++) {
                    if (sel.options[oi].value == kwId) { already = true; break; }
                }
                if (!already) {
                    sel.add(new Option(phpHatNames[kwId], kwId));
                }
            }
        }
    }
...
function addStarMerc(table_id, id)
{
    ...
    */
    ...
    var fields = ['Name', 'Hiring cost', 'Additional skills', 'MVP', 'Cp', 'TD', 'Int', 'BH', 'SI', 'Ki', 'Misc SPP', 'Remove'];
    ...
    if (id == ID_MERCS) {
        ...
        x[10] = '<input type="text" onChange="numError(this);" size="1" maxlength="2" value="0" id="misc'+idm+'" name="misc'+idm+'">';
        x[11] = '<a href="javascript:void(0)" onclick="rmStarMerc('+ID_MERCS+','+MERC_CNT+');">Remove</a>';
        MERC_CNT++;
    }
    // Star?
    else {
        ...
        // Let the OPPOSING team's Hatred dropdowns pick up this star's keyword(s) immediately, without needing the
        // report saved and reloaded first.
        addHatredKeywordsToOpponent(table_id2, phpStars[pos]['keyword']);
        ...
        x[10] = '<input type="text" onChange="numError(this);" size="1" maxlength="2" id="misc_'+id+'" name="misc_'+id+'" value="0">';
        x[11] = '<a href="javascript:void(0)" onclick="rmStarMerc('+id+',0);">Remove</a>';
            
        // Remove star from add-list drop-downs.
        for (i = 0; i < stars.options.length; i++) {
            if (stars.options[i].value == id) {
                stars.remove(i);
                break;
            }
        }
        for (i = 0; i < stars2.options.length; i++) {
            if (stars2.options[i].value == id) {
                stars2.remove(i);
                break;
            }
        }
