...
class HTMLOUT
{
	...
	public static function standings($obj, $node, $node_id, array $opts) {
		...
		// FIX: For team standings with ALL leagues selected, ensure only teams with games played are shown
		// BUT NOT on coach team lists where we want to show all teams regardless of games played
		if (!$_COACH_TEAM_LIST && $obj == STATS_TEAM && $sel_node === false && $sel_node_id === false && $sel_ff_field == 'mv_played' && $sel_ff_limit == '0') {
			$sel_ff_limit = '1';
		}
