...
class SQLCore
{
	...
	*/
	public static function syncGameData() {
		global $core_tables, $DEA, $stars, $skillarray, $specialrulesarray, $playerkeywordsarray  ;
		$players   = 'game_data_players';
		$races     = 'races';
		$starstbl  = 'game_data_stars';
		$skillstbl = 'game_data_skills';
		$specialrulestbl = 'special_rules';
		$playerkeywordstbl = 'player_keywords';
		$status = true;
		...
		$status &= Table::createTable($playerkeywordstbl,$core_tables[$playerkeywordstbl]);

		foreach ($DEA as $race_name => $race_details) {
			$query = "INSERT INTO $races(race_id, name, cost_rr, bigguy_qty, ca_qty, bz_qty, rn_qty, th_qty, bk_qty, sp_qty, format, tier, team_league, special_rules, fav_rules) VALUES (".$race_details['other']['race_id'].", '".mysql_real_escape_string($race_name)."', ".$race_details['other']['rr_cost'].", ".$race_details['other']['bigguy_qty'].", ".$race_details['other']['ca_qty'].", ".$race_details['other']['bz_qty'].", ".$race_details['other']['rn_qty'].", ".$race_details['other']['th_qty'].", ".$race_details['other']['bk_qty'].", ".$race_details['other']['sp_qty'].", '".$race_details['other']['format']."', ".$race_details['other']['tier'].", '".implode(',',$race_details['other']['team_league'])."', '".implode(',',$race_details['other']['special_rules'])."', '".implode(',',$race_details['other']['fav_rules'])."')";
		...
		foreach ($stars as $star_name => $SD) {
			$query = "INSERT INTO $starstbl(star_id, name, cost, races, teamrules, ma,st,ag,pa,av, skills, special,specialdesc,megastar,keywords) VALUES (
				$SD[id], '".mysql_real_escape_string($star_name)."', $SD[cost], '".implode(',', $SD['races'])."', '".implode(',', $SD['teamrules'])."', $SD[ma],$SD[st],$SD[ag],$SD[pa],$SD[av], '".implode(',', $SD['def'])."', '".implode(',', $SD['special'])."', '".mysql_real_escape_string($SD['specialdesc'])."', $SD[megastar], '".implode(',', $SD['keyword'])."'
			)";
			$status = mysql_query($query);
		}
		...
		foreach ($playerkeywordsarray as $kcat => $keywords) {
			foreach ($keywords as $kid => $k) {
				$status &= mysql_query("INSERT INTO $playerkeywordstbl(keyword_id, name, cat) VALUES ($kid, '".mysql_real_escape_string($k)."', '$kcat')");
			}
		}
	...
	public static function mkHRS(array $HRSs) {
		...
		$allowed_fields = array(
			'mvp', 'cp', 'td', 'intcpt', 'bh', 'si', 'ki', 'misc', 'cas', 'tdcas', 'smp', 'bonus', 'elo',	'gf', 'ga', 'sdiff', 'tcasf', 'tcasa', 'tcdiff', 'won', 'lost', 'draw', 'swon', 'slost', 'sdraw', 'played', 'win_pct',
		);
	...
	public static function installProcsAndFuncs($install = true) {
		...
		$routines = array(
			...
			BEGIN
				...
				DECLARE cnt_skills_random, cnt_skills_norm, cnt_skills_doub, cnt_skills_elite, cnt_skills TINYINT UNSIGNED;
				...
				SELECT
					...
					IFNULL(SUM(IF(inj = '.MA.', 1, 0) + IF(agn = '.MA.', 1, 0)), 0),
					IFNULL(SUM(IF(inj = '.AV.', 1, 0) + IF(agn = '.AV.', 1, 0)), 0),
					IFNULL(SUM(IF(inj = '.AG.', 1, 0) + IF(agn = '.AG.', 1, 0)), 0),
					IFNULL(SUM(IF(inj = '.PA.', 1, 0) + IF(agn = '.PA.', 1, 0)), 0),
					IFNULL(SUM(IF(inj = '.ST.', 1, 0) + IF(agn = '.ST.', 1, 0)), 0)
				INTO
