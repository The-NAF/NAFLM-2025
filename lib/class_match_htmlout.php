...
$T_MOUT_ACH = array_combine($T_PMD_ACH, array('MVP','Cp','Td','Int','BH','SI','Ki','Misc SPP',));
$T_MOUT_IR  = array_combine($T_PMD_IR, array('IR1 D1','IR1 D2','IR2 D1','IR2 D2','IR3 D1','IR3 D2',));
$T_MOUT_INJ = array_combine($T_PMD_INJ, array('Inj','Ageing'));
$T_MOUT_HAT = array_combine($T_PMD_HAT, array('Hatred'));
...
class Match_HTMLOUT extends Match
{
	...
	public static function report() {
		...
		if (isset($_POST['button']) && $ALLOW_EDIT) {
			...
			// SECOND, same as above for zombie but for skeletons instead.
			foreach (array(1 => $team1, 2 => $team2) as $id => $t) {
				if (in_array($t->f_race_id, $racesHasSkelemancer) && isset($_POST["t${id}skeleton"])) {
					$pos_id = $DEA[$t->f_rname]['players']['Skeleton Lineman']['pos_id'];
					list($exitStatus,$pid) = Player::create(
						array(
							'nr' => $t->getFreePlayerNr(),
							'f_pos_id' => $pos_id,
					...
					/*
						Knowing the new skeleton's PID we relocate the skeleton match data to regular player data - this allows us to use the same loop for submitting the skeleton's match data.
					*/
					foreach ($T_PMD__ENTRY_EXPECTED as $f) {
						$postName = "${f}_t${id}skeleton";
						$_POST["${f}_$pid"] = isset($_POST[$postName]) ? $_POST[$postName] : 0;
						unset($_POST[$postName]);
					}
				}
			}
			// THIRD, look for raised rotters too, do same as above with skeleton/zombies (except they are a journeyman that must be bought, not free)...
			foreach (array(1 => $team1, 2 => $team2) as $id => $t) {
			...
			// FOURTH, look for summoned thralls too, do same as above with zombies...
			foreach (array(1 => $team1, 2 => $team2) as $id => $t) {
			...
			foreach (array(1 => $team1, 2 => $team2) as $id => $t) {
				...
				foreach (array_values($playerFields) as $f) {
				   ...
				   switch(strtolower(str_replace(' ', '', $f)))
				   {
					   ...
					   case 'ageing': $header_text = $lng->getTrn('matches/report/ageing'); break;
					   case 'hatred': $header_text = $lng->getTrn('matches/report/hatred'); break;
				...
				global $racesHasSkelemancer;
				if (in_array($t->f_race_id, $racesHasSkelemancer)) {
					echo "<hr style='width:200px;float:left;'><br>
				...
				<table style='border-spacing: 0px 10px;'>
					<tr><td align="left" valign="top">
						...
						<select id="stars_<?php echo $id;?>" <?php echo $DIS; ?>>
							<?php
							if (strlen($t->getLeaguechosen()) == 0) {
								$teamdefrules = explode(",", $t->getLeagueoptions());
							} else {
								$teamdefrules = explode(",", $t->getLeaguechosen());
							}
							$teamdfavrule  = explode(",", $t->getFavrulechosen());
							$teamdspecrule = explode(",", $t->getTeamrules());
							$allteamrules  = array_merge($teamdefrules, $teamdfavrule, $teamdspecrule);
							...
							foreach ($stars as $s => $d) {
								...
								if (isset($rules['banned_stars']) && !empty($rules['banned_stars'])) {
									...
									if (in_array($d['id'], $bannedStarsArray)) continue;
								}
								// Filter megastars
								if (!($rules['megastars'] == 0 || ($d['megastar'] == 0 && $rules['megastars'] == 1))) continue;
								// Filter by team special rules
								$starcanplay = array_intersect($d['teamrules'], $allteamrules);
								if (empty($starcanplay)) continue;

								echo "<option value='$d[id]'>$s</option>\n";
							}
							?>
						</select>
					</td></tr>
